#before creating RDS, we need to create subnets group
...
# and also for the elastic cache
...
resource "aws_db_instance" "vprofile-rds" {
  ...
  skip_final_snapshot = true #we are training, and this is preventing the RDS to keep a snapshot when destroyed. not to be used in production
  db_subnet_group_name = aws_db_subnet_group.vprofile-rds-subgrp.name #name and not id here
...
resource "aws_mq_broker" "vprofile-rnq" {
